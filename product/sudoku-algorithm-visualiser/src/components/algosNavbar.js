                            ...
                            {isAlgorithmsOpen && (
                                <ul className="absolute mt-2 bg-gray-800 border rounded">
                                    {[["Backtracking (BT)", "Backtracking"], ["Constraint Propagation (BT)", "Constraint-Propagation"]].map(([algo, html], index) => (
                                        <button key={index} onClick={() => window.location.href = "/"+ html} className="w-full px-4 py-2 hover:bg-gray-700 text-nowrap">
                                            <a>{algo}</a>
                                        </button>
                                    ))}
                                </ul>
                            ...
                            {isSpeedOpen && (
                                <ul className="absolute mt-2 bg-gray-800 border rounded">
                                    {[["Slow", 50], ["Medium", 40], ["Fast", 25], ["Fastest", 0]].map(([label, speed], index) => (
                                        <button key={index} onClick={() => {changeSpeed(speed); setSpeedType(label)}} className="w-full px-4 py-2 hover:bg-gray-700">
                                            <a>{label}</a>
                                        </button>
                                    ))}
                                </ul>
