...
const Grid = ({ rows = 9, cols = 9 }) => {
        ...
        for (let j = 0; j < cols; j++) {
            ...
            gridItems.push(
                <div className={className} key={`${index}`}>
                    <input id="boxcontent" readOnly={false} type="number" min="1" max="9" data-index={index} data-row={i} data-col={j} required className=" text-xl bg-[#1b212c] h-[80%] w-[80%] place-items-center [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
    ...
    return (
        <div>
            ...
            <div className="grid bg-[#1b212c] border-solid rounded border-4 mx-auto"
            style={{
